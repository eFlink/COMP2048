...
class RunLengthEncodedParser:
    ...
    def populate_pattern(self, pattern_raw, size_x, size_y, default_cell='b'):
        ...
        for y in range(len(pattern_rows)):
            row = []
            # append to a row rather than directly to account for the number of pattern rows diverging
            # from the number of actual rows in accordance with {number}$ syntax
            tmp_num_str = ""
            ...
            # Credit to Jed Willick for finding the bug. 
            if tmp_num_str:
